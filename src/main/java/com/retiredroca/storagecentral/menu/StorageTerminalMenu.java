...
public class StorageTerminalMenu extends AbstractContainerMenu {
    ...
    public StorageTerminalMenu(int containerId, Inventory playerInventory, StorageTerminalBlockEntity terminal) {
        super(Registration.STORAGE_TERMINAL_MENU.get(), containerId);
    ...
    public static StorageTerminalMenu fromNetwork(int containerId, Inventory playerInventory,
        ...
        StorageTerminalMenu menu = new StorageTerminalMenu(containerId, playerInventory, pos, null);
        return menu;
    }
    ...
    private ItemStack insertBestFit(IItemHandler handler, ItemStack stack) {
        ItemStack remaining = stack.copy();
    ...
    public void doExtract(Player player, ItemStack requested, int mode) {
        ...
        for (IItemHandler handler : terminal.getScannedHandlers()) {
            for (int slot = 0; slot < handler.getSlots(); slot++) {
                if (output.getCount() >= desired) {
                    break;
                }
                ItemStack inSlot = handler.getStackInSlot(slot);
                if (!inSlot.isEmpty() && ItemStack.isSameItemSameComponents(inSlot, requested)) {
                    int need = desired - output.getCount();
                    ItemStack got = handler.extractItem(slot, need, false);
                    if (!got.isEmpty()) {
                        if (output.isEmpty()) {
                            output = got;
                        } else {
                            output.grow(got.getCount());
                        }
                    }
                }
            }
